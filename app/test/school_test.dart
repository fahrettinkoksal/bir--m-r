...
void main() {
  group('Eğitim durumu veride tutulur', () {
    test('doğumda okula başlanmamıştır ve öğrenci değildir', () {
      for (int seed = 0; seed < 30; seed++) {
        final GameState state = LifeGenerator.seeded(seed)
    ...
    test('notu yeterli öğrenci her yaşta bir sınıf ilerler', () {
      ...
      for (int sinif = 1; sinif <= LifeProgression.lastGrade; sinif++) {
        ...
        expect(controller.state!.education.grade, sinif,
    ...
    test('eğitim durumu yaştan türetilmez', () {
      ...
      final GameState state = LifeGenerator.seeded(1)
          .generate(mode: StartMode.tamamenRastgele)
          .copyWith(player: LifeGenerator.seeded(1)
              .generate(mode: StartMode.tamamenRastgele)
              .player
              .copyWith(age: 10));
      ...
      for (int seed = 0; seed < 30; seed++) {
        for (final int age in <int>[3, 7, 12, 20]) {
          ...
          final GameState yasli =
              state.copyWith(player: state.player.copyWith(age: age));
          expect(yasli.education.isStudent, isFalse);
          expect(engine.openingEvent(yasli, Random(seed)), isNull,
  ...
  group('Okul arkadaşı kalıcı bir kişidir', () {
    ({GameState state, String friendId}) withFriend(int seed) {
      ...
      final Person friend = sonra.people
          .firstWhere((Person p) => p.relation == RelationType.arkadas);
    ...
    test('tanışma seçimi gerçek bir kişi kaydı oluşturur', () {
      for (int seed = 0; seed < 20; seed++) {
        ...
        expect(friend.relation.kanBagi, isFalse, reason: 'Arkadaş akraba değil');
        expect(friend.inPlayerHousehold, isFalse,
            reason: 'Arkadaş otomatik hanede sayılmaz');
        expect(friend.bond, greaterThan(0));
        expect(sonuc.state.storyFlags, contains(StoryFlags.okuldaArkadasEdindi));
      }
    });
    ...
    test('arkadaşın kimliği yaş alındıkça ve olaylar boyunca korunur', () {
      ...
      // Arkadaş edinilene kadar tanışma seçeneğini tercih ederek ilerle.
      String? friendId;
      String? friendName;
      for (int i = 0; i < 40 && friendId == null; i++) {
        resolvePendingEvents(controller, preferChoiceId: 'tanis');
        controller.ageUp();
        resolvePendingEvents(controller, preferChoiceId: 'tanis');
      ...
      final int bondOnce = controller.state!.personById(friendId!)!.bond;
      for (int i = 0; i < 10; i++) {
        resolvePendingEvents(controller, preferChoiceId: 'tanis');
        controller.ageUp();
      }
      ...
      final Person sonra = controller.state!.personById(friendId)!;
      ...
      expect(controller.state!.personById(friendId), isNotNull);
    });
    ...
    test('yalnızca bir okul arkadaşı oluşur, kimlikler çakışmaz', () {
      ...
      final Set<String> ids = sonuc.state.people.map((Person p) => p.id).toSet();
  ...
  group('Seçimler hatırlanır', () {
    ...
    test('yardım etmek ileride ayrı bir olay açar', () {
      final EventEngine yardimEngine =
          schoolEngine(<String>['arkadas_odev_yardimi']);
      final EventEngine karsilikEngine =
          schoolEngine(<String>['yardimin_karsiligi']);

      final ({GameState state, String friendId}) baslangic =
          friendAtGrade(5, 5);
      // Yardım olayı çıkmadan önce karşılık olayı açılmaz.
      expect(karsilikEngine.openingEvent(baslangic.state, Random(1)), isNull);

      final ActiveEvent? yardim =
          yardimEngine.openingEvent(baslangic.state, Random(1));
      expect(yardim!.eventId, 'arkadas_odev_yardimi');
      expect(yardim.personId, baslangic.friendId,
          reason: 'Olay gerçek arkadaşla kurulmalı');

      final int bondOnce =
          baslangic.state.personById(baslangic.friendId)!.bond;
      ...
      final ActiveEvent? karsilik =
          karsilikEngine.openingEvent(yardimEtti, Random(1));
      expect(karsilik, isNotNull, reason: 'Geçmiş seçim hatırlanmalı');
      expect(karsilik!.eventId, 'yardimin_karsiligi');
      expect(karsilik.personId, baslangic.friendId,
          reason: 'Karşılık aynı arkadaştan gelmeli');
      expect(karsilik.text, contains(
        yardimEtti.personById(baslangic.friendId)!.firstName,
      ));
    });
    ...
    test('yardım etmemek o devamı açmaz ve ilişkiyi zayıflatır', () {
      ...
      expect(reddetti.storyFlags, isNot(contains(StoryFlags.arkadasaYardimEtti)));
      expect(
        reddetti.personById(baslangic.friendId)!.bond,
        lessThan(bondOnce),
      );
    ...
    test('derste söz almak karakter değerlerini değiştirir', () {
      ...
      expect(sonra.player.stats.intelligence,
          greaterThan(state.player.stats.intelligence));
      expect(sonra.player.stats.charisma,
  ...
  group('Diğer sistemler bozulmadı', () {
    test('aile, romantik ilişki ve yaş alma birlikte çalışır', () {
      for (int seed = 0; seed < 15; seed++) {
        ...
        expect(controller.state!.people.length,
            greaterThanOrEqualTo(kisiOnce));
